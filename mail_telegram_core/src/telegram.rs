...
async fn send_message_in_telegram(
    bot: &Bot,
    mail: db::Mail,
) -> Result<db::Mail, Box<dyn std::error::Error>> {
    let path = pdf::get_pdf_path_by_id(&mail.message_id)?;
    let pdf_file = InputFile::file(path).file_name(format!("{}.pdf", mail.subject.clone()));
    let _ = bot
        .send_document(ChatId(EnvVars::telegram_my_chat_id()), pdf_file)
        .await?;
    Ok(mail)
}
...
pub async fn send_mails_in_telegram(
    ...
    let unsent_mails = mails_db.fetch_unsent_mails().await?;
    ...
    mails_db
        .update_mails_as_sent(sent_messages.into_iter())
        .await?;
